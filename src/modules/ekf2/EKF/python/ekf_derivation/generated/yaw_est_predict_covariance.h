...
void YawEstPredictCovariance(const matrix::Matrix<Scalar, 3, 1>& state,
  ...
  const Scalar _tmp7 = _tmp0 * d_ang;
  const Scalar _tmp8 = _tmp1 + _tmp7;
  const Scalar _tmp9 = _tmp0 - _tmp1 * d_ang;
  const Scalar _tmp10 = -_tmp8 * (-_tmp1 - _tmp7) + std::pow(_tmp9, Scalar(2));
  ...
  if (P_new != nullptr) {
    ...
    _p_new(2, 1) = 0;
    _p_new(0, 2) = _tmp10 * _tmp3;
    _p_new(1, 2) = _tmp10 * _tmp6;
    _p_new(2, 2) = P(2, 2) * std::pow(_tmp10, Scalar(2)) +
                   d_ang_var * std::pow(Scalar(-_tmp0 * _tmp9 - _tmp1 * _tmp8), Scalar(2));
  }
